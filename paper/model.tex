...
where
\begin{align}
\widehat{\truth} &= \left\lfloor \phi \cdot \sigma + 0.5 \right\rfloor, \quad \widehat{\truth} \in \{0,1\}\\
...
where $\omega$, $\gamma$ and $\delta$ are similarly modeled as the $\bias$ in Equation 2,
$\omega + \gamma = 1$, $\omega + \delta = 1$ and $k \in [0,1]$
...
\begin{equation}
R_i^{t+1} = R_i^t 
+ \eta \cdot \mathbf{1}\{a_i^t = 1 \land \theta_t = 1\}
- \lambda \cdot \mathbf{1}\{a_i^t = 1 \land \theta_t = 0\}
\end{equation}
